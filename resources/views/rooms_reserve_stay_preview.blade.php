...
@section('content')
  ...
  <section id="hero_rooms" class="d-flex align-items-center">
    <div class="container text-center">
      <h1 class="mt-7 pt-5">お部屋予約確認(宿泊)</h1>
    </div>
  </section><!-- End Hero -->
      ...
      <div class="container">
        <div class="row">
          <div class="col-lg-8 col-md-8 mx-auto">
            ...
            <h3 class="bold mt-5">チェックイン:</h3>
            <h3 class="bold">{{$get_query1->date}}</h3>
            <?php
              $checkin = $get_query1->date;
              $stay_num = $total - 1;
              $checkout = date('Y-m-d', strtotime($checkin.'+'.$stay_num.' day'));
            ?>
            <h3 class="bold mt-5">チェックアウト:</h3>
            <h3 class="bold">{{$checkout}}</h3>
            <h3 class="bold mt-5">滞在日数:</h3>
            <h3 class="bold">{{$total}}日間</h3>
            <h3 class="bold mt-5">チェックイン時間:</h3>
            <h3 class="bold">{{$get_query1->checkin_time}}</h3>
            ...
            <form id="payment-form" action="/roomsreserve_stay/confirm/{{$room_id}}/{{$add_data_id}}/{{$get_query1->name}}/{{$get_query1->email}}" enctype="multipart/form-data" method="POST">
              @csrf
              <table class="mt-4">
                <tr>
              ...
              <div class="text-center">
                <a href="/roomsreserve_stay/retly/{{$room_id}}/{{$add_data_id}}/{{$get_query1->name}}/{{$get_query1->email}}" class="btn btn-success">予約設定をやり直す</a>
